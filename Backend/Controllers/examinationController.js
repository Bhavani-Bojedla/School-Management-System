...
const getExaminationByClass=async(req,res)=>{
    try{
       ...
       const examinations=await Examination.find({school:schoolId,class:classId});
...
const updateExaminationWithId=async(req,res)=>{
    try{
       ...
       await Examination.findByIdAndUpdate({_id:examinationId,school:schoolId},{$set:{
          examDate:date,
          subject:subjectId,
          examType:examType
       }})
...
const deleteExaminationWithId=async(req,res)=>{
    try{
       ...
       res.status(200).json({success:true,message:"Examination is deleted successfully."})
...
module.exports={newExamination,getAllExaminations,getExaminationByClass,updateExaminationWithId,deleteExaminationWithId}