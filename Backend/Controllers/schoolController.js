...
const getSchools=async(req,res)=>{
  try {
    ...
    res.status(200).json({
      success:true,
      message:"Succcess in fetching all schools.",schools
    })
  } catch (error) {
    res.status(500).json({
      success:false,
  ...
  try {
    ...
    form.parse(req, async (e, fields, files) => {
      ...
      if(files.image){
        ...
        res.status(200).json({
          success:true,
          message:"School Updates successfully",School
        })
      }
    });
  } catch (error) {
...
module.exports = { registerSchool,loginSchool,getSchools ,getSchoolOwnData,updateSchool};
